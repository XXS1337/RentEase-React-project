...
export const homeLoader = async () => {
  ...
  let userFavorites: string[] = []; // Initialize an empty array for user favorites
  ...
  const flats = flatsSnapshot.docs
    .map((doc) => ({
      ...
      image: `/flats/${doc.data().image}`,
      createdAt: doc.data().createdAt || Date.now(), // Generate image path
    }))
...
const Home: React.FC = () => {
  const { flats: initialFlats, userId } = useLoaderData() as { flats: any[]; userId: string | null }; // Load flats and user ID
  ...
  const sortFlats = (flatsToSort: any[], option: string) => {
