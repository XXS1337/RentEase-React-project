...
const ProtectedRoute: React.FC<ProtectedRouteProps> = ({ adminOnly = false }) => {
  const [isLoading, setIsLoading] = useState<boolean>(true); // State to indicate loading status
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      try {
        ...
        if (userDoc.exists()) {
          const userData = userDoc.data() as { isAdmin?: boolean }; // Ensure type safety for Firestore data
